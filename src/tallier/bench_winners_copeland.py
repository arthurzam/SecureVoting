...
async def run(clique_size: int, candidates_count: int, runs: int):
    ...
    mock_election = Election(uuid4(), "Arthur", "a@a.com", ElectionType.copeland, candidates, 1, p, 5)
    ...
    ballot = build_ballot(tuple(range(candidates_count)), is_maximin=False)
    ...
    async def code(t: MpcWinner, votes_vector: tuple[int, ...]):
        votes_vector = await t.copeland_scores(0, candidates_count, 1, 2, votes_vector)
...
async def main():
    ...
    import csv
    with open("/tmp/bench_winners_copeland.csv", "w") as f:
        writer = csv.writer(f)
